...
let maxBookCode = () => {
  ...
  return Math.max.call(null, arr, 0);
};
// 把内存数据写入文件
...
// 实现单选修改某一项的状态
exports.setcheckboxstatus = (req, res) => {
  let id = req.params.id;
  let bol = Boolean(Number(req.params.status));
  console.log(data, id);
  data.some((item, index) => {
    if (id == item.id) {
      // 删除数组的一项数据
      data[index].completed = bol;
